resource "aws_s3_bucket" "bucket" {
  count  = length(keys(var.bucketnames))
  bucket = lower(var.bucketnames[count.index])
  acl    = element(var.bucket_acls, count.index)
  policy = var.attach_policy ? (file("../../../variables/${var.environment}/global/s3/policies/${var.policy_filename}")) : ""
      ...
      content{
        ...
        rules {
          ...
          destination {
            ...
            replica_kms_key_id = replication_configuration.value.replication_key_arn
          ...
          account_id = replication_configuration.value.account_id
