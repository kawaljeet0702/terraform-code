...
resource "aws_db_subnet_group" "private-db-subnetgroup" {
  count       = var.rds_count
  name        = lower("${lower(var.tags_name)}${var.startindex}-db-sng${count.index}")
...
module "ec2_sg" {
  ...
  vpc_id                            = var.vpc_id
  region                            = var.region
  description                       = "${lower(var.tags_name)}${var.startindex}-postgres-sg"
  ...
  tags_department                   = var.tags_department
  tags_department_data_owner        = var.tags_department_data_owner
  tags_name                         = "${lower(var.tags_name)}${var.startindex}-postgres-sg"
  }
